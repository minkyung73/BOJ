...
// 영원
public class DAEJEON05_JAVA {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "DAEJEON05_JAVA";
		...
		try {
			...
			while (socket != null) {
				...
				for (int i = (order==1) ? 1 : 2; i <= 6; i+=2) {
					if(i==6) i=5;
					w = Math.abs(whiteBall_x - balls[i][0]);
					h = Math.abs(whiteBall_y - balls[i][1]);
//					System.out.println(w + " " + h + " " + distance);
					if(balls[i][0]!=-1 && Math.sqrt((w * w) + (h * h)) < minDist) {
						if(targetBall_x != -1 && i==5) break;
...
//					}
				...
				float update_w = (float) (5.73f / holeDist * holeWidth);
				float update_h = (float) (5.73f / holeDist * holeHeight);
